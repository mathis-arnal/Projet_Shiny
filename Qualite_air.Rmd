...
Les données météorologiques issues de Meteostat que nous avons présenté
...
anova(mod_complet, mod_2, test="Chisq")

summary(mod_2)

## On recommence avec un autre modèle où on enlève pres
mod_3 <- glm(qualite_air_groupe ~ tavg + prcp + wspd + Vent_x + Vent_y, data = dt_qualite_air, family = "binomial")
...
Regardons en detail les effets de ces variables

```{r}
summary(mod_3)
```

D'après ce résumé, on retrouve que

-   une augmentation de la température moyenne (tavg) est associée à une
    augmentation de la probabilité de mauvaise qualité de l'air par
    rapport à la bonne qualité de l'air

-   à l'inverse, une augmentation des précipitation et de la vitesse du
    vent est associée à une augmentation de la probabilité de bonne
    qualité de l'air

-   Pour les valeurs de vents, on reprend les transformations
    trigonométriques qui nous avaient permis de creer ces variables, et
    on retoruve qu'une augmentation de vent_x et de vent_y, donc un vent
    vers l'est et le nord est associé à une augmentation de la
