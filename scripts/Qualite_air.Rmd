...
En comparant avec le graphique des variables, ce sont les varibles de
température qui participent à la création de l'axe 1. Alors les jours
sont bien séparé selon leur temprétaure (été/hiver), mais ne semble pas
nous indiquer particulièrement sur la qualité de l'air.

En effet, on vois sur le graphique des individus colorés par groupe de
qualité de l'air que le groupe 1-2 se réparti le long des deux première
dimension de manière assez homogène. Les individus des groupes 3-4 sont
legèrement plus vers le haut du graphique et les individus du groupe 1-2
plutot vers le bas, mais les deux groupes et se répartisse de manière
homogène le long de la première dimension. On retouve cela avec le
barycentre des groupes 1-2 et 3-4 qui sont environ aux même coordonées
sur la première dimension mais qui sont un peu séparés sur la deuxième.
...
dimension. Le groupe 1-2, de bonne qualité de l'air est ainsi d'avantage
lié à des fortes précipitations et des direction de vent particulières,
...
Avec le critène d'AIC, on trouve un modèle à 11 paramètres, dont les
effets sont tous significatifs au seuil 0.10, et avec 8 paramètres dont
les effets sont significatifs au seuil 0.05.
...
-   (\*\*\*) Les deux paramètres qui semblent expliquer au mieux la
    ...
    y, ie. vers le nord. Une augmentation et de vent_y, donc un vent le
    nord est associé à une diminution de la augmetnation de bonne
    qualité de l'air. Pour la vitesse du vent, une augmentation de la
    vitesse du vent, est associé à une diminution de la probabilité de
...
-   (\*\*) Les autres variables qui ont des effets sont significatifs
    ...
    (vent_x_lag_1). L'interprétation des paramètres par paramètres est
...
-   En plus du vent, la précipitation du jour et la tempétature du jour
    expliquent la qualité de l'air dans notre modèle: de fortes
    précipitation et des petites températures maximales sont associées à
